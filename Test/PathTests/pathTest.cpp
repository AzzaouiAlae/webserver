...
{
	try {
		DeprecatedTokenizing token("Test/PathTests/ConfigFiles/TestRootPath.conf");
		token.split_tokens();
		DeprecatedValidation valid(token.get_tokens());
...
{
	try {
		DeprecatedTokenizing token("Test/PathTests/ConfigFiles/TestLocationPath.conf");
		token.split_tokens();
		DeprecatedValidation valid(token.get_tokens());
		valid.CheckValidation();
...
{
	try {
		DeprecatedTokenizing token("Test/PathTests/ConfigFiles/TestLocationRootPath.conf");
		token.split_tokens();
		DeprecatedValidation valid(token.get_tokens());
		valid.CheckValidation();
