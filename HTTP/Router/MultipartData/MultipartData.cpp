...
MultipartData::MultipartData(const string &boundary)
    : _boundary(boundary), _status(eParsingBoundary), _parseIdx(0)
{
    _delimiter = "\r\n--" + _boundary;
    _current.reset();
}
...
void MultipartData::_scanBody(char *buf, size_t len)
{
    ...
    if (!_partialMatch.empty())
    {
        ...
        size_t compareLen = min(needed, len - _parseIdx);

        if (equal(buf + _parseIdx, buf + _parseIdx + compareLen, _delimiter.begin() + _partialMatch.length()))
        {
            _partialMatch.append(buf + _parseIdx, compareLen);
            _parseIdx += compareLen;

            if (_partialMatch == _delimiter)
            {
                ...
                if (_parseIdx + 2 <= len && buf[_parseIdx] == '-' && buf[_parseIdx + 1] == '-')
                {
                    _status = eComplete;
                    _parseIdx += 2;
                }
        ...
        {
            _emitBodyPart(0, 0, false);
            _partialMatch.clear();
        }
    }

    char *it = search(buf + _parseIdx, buf + len, _delimiter.begin(), _delimiter.end());
