...
//  --- Checks one location ---
void DeprecatedValidation::validateLocation(AST<string> &locationNode, LocationSeen &seen)
{
...
void DeprecatedValidation::validateCgiPass(AST<string> &node, LocationSeen &seen)
{
	...
	if (ext.empty() || ext[0] != '.')
		Error::ThrowError("Config Error: cgi_pass extension must start with a dot — got '" + ext + "'");
...
void DeprecatedValidation::checkArgCount(AST<string> &node, size_t min, size_t max, const string &name)
{
...
void DeprecatedValidation::requireArgs(AST<string> &node, const string &name)
{
	if (node.GetArguments().empty())
